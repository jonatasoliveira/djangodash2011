...
def create_db_file(engine,dir_db_files,v_domain):
    ...
    domains_result = engine.execute(domains_table.select().where("domain=:domain"),domain=v_domain)
...
def create_zone_file(engine,dir_db_files,verbose):
